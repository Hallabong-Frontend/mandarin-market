...
- **작성자 여부에 따른 모달 분기 처리**
  - 작성자 → 수정 / 삭제
  - 타 사용자 → 신고
- **Profile.jsx 4개의 핵심 컴포넌트로 분리**
  - ProfileInfo.jsx: 상단 프로필 정보 및 버튼 영역
  - ProfileProductList.jsx: "판매 중인 상품" 영역
  - ProfilePostSection.jsx: 리스트/앨범 보기 토글 버튼과 사용자의 게시글 영역
  - ProfileSettingsModal.jsx: "설정 및 개인정보" 커스텀 모달
...
- 피드, 게시글 CRUD, 이미지 최대 3장, 좋아요, 댓글

**핵심 구현**

- **Firestore 기반 대댓글 계층 구조 및 댓글 고정 구현하고 피드 UI 및 하단 탭 메뉴 구현을 담당하고, PostDetail에서 대댓글/댓글 고정/이모지 리액션 등 세밀한 인터랙션 기능을 구현했습니다. 전체 프로젝트 JSDoc 문서화 및 QA를 담당했습니다.**
...
React 기반 SNS 서비스에서 사용자 관리(Profile), 채팅(Chat), 게시글(Feed) 관련 핵심 기능을 구현하고 UI/UX 개선 및 버그 수정을 담당했습니다.
...
프로젝트에서 여러 화면에 공통으로 사용되는 `Header` 컴포넌트를 구현하면서 헤더 구조를 어떤 방식으로 설계하는 것이 적절한지 고민했습니다.

초기에는 `type` 값을 기준으로 내부에서 조건 분기를 통해 화면별 헤더 UI를 재사용하는 방식을 사용했습니다.
...
이 방식은 구현이 단순하고 빠르게 여러 화면에 적용할 수 있다는 장점이 있었습니다.  
특히 프로젝트 초반에는 화면별 헤더 패턴이 비교적 명확했기 때문에 생산성 측면에서 효율적이었습니다.

하지만 개발이 진행되면서 헤더에 들어가는 요소가 점점 다양해지고  
뒤로가기 버튼, 로고, 타이틀, 검색 버튼, 더보기 버튼 등 여러 요소를 조합해야 하는 상황이 생기면서  
문자열 기반 분기 방식은 확장성 측면에서 한계가 있을 수 있다고 판단했습니다.

이 과정에서 `Compound Component` 패턴을 대안으로 검토했습니다.
...
이 방식은 Header 내부 요소를 각각 독립적인 컴포넌트로 분리하여 필요한 요소를 조합하는 방식으로 사용할 수 있습니다.

문자열 기반 분기 대신 컴포넌트 조합 방식으로 UI를 구성할 수 있기 때문에 다음과 같은 장점이 있다고 판단했습니다.

- 문자열 기반 조건 분기를 줄일 수 있음
- 화면별 요구사항에 따라 Header 구성을 유연하게 조합 가능
- 컴포넌트 단위 재사용성과 확장성이 높아짐

하지만 프로젝트가 이미 **개발 후반 단계**였고 기존 Header 구조가 안정적으로 동작하고 있었기 때문에  
전체 페이지에서 사용 중인 Header 호출 방식을 수정하는 것은 리팩토링 비용이 크다고 판단했습니다.

특히 다음과 같은 이유로 이번 프로젝트에서는 구조 변경을 진행하지 않았습니다.

- 여러 페이지의 Header 사용 코드를 함께 수정해야 함
- 테스트 및 QA 범위가 크게 증가함
- 현재 프로젝트 규모에서는 구조 변경의 실익이 크지 않음

따라서 이번 프로젝트에서는 **기존의 분기형 Header 구조를 유지**하기로 결정했습니다.

대신 향후 프로젝트 규모가 커지거나 Header 조합 방식이 더 복잡해질 경우에는  
`Compound Component` 패턴을 적용하는 것이 더 적절한 구조라고 판단했습니다.
...
### 6. AI 기반 초기 설계 시 SVG 하드 코딩으로 인한 리팩토링 어려움

**문제**  
프로젝트 초기에 AI를 활용하여 UI의 큰 틀을 빠르게 생성했습니다. 하지만 이 과정에서 AI가 모든 아이콘(SVG)을 긴 하드 코딩 태그(`<svg>...</svg>`)로 직접 삽입하여 코드를 생성했습니다. 이후 피그마에서 추출한 공식 에셋 파일(`.svg`)로 디자인을 교체해야 할 때 코드가 너무 방대해지고 얽혀 있어, 스타일(styled-components)을 다시 입히고 에셋을 분리하는 데 큰 어려움이 있었습니다.

**해결**  
수많은 컴포넌트 내에 하드 코딩된 `<svg>` 태그들을 일일이 찾아내어 피그마에서 추출한 공식 에셋 파일(`.svg`)로 교체, 이를 styled-components에서 불러와서 속성을 제어할 수 있도록 구조를 전면 개편했습니다.

**결과**

- SVG 아이콘이 외부 파일 혹은 모듈로 분리되어 컴포넌트 가독성 대폭 향상
- 피그마 에셋 업데이트 시 교체 작업이 수월해짐
- 아이콘 재사용성이 높아져 유지보수 비용 감소
...
> 약 2주간 팀 리더로서 기능 구현과 협업 조율을 동시에 담당했다. 초기에는 게시글 작성, 이미지 업로드 등 기능을 빠르게 쌓는 데 집중했고, 이후 `Profile.jsx`, `ChatList.jsx`, `Header` 컴포넌트를 직접 리팩토링하며 컴포넌트 설계와 책임 분리의 중요성을 팀에 공유했다. Firebase Firestore 기반 실시간 채팅은 REST API와 전혀 다른 사고방식을 요구했지만, 구조를 직접 설계하고 팀원들에게 공유하는 과정을 통해 팀 전체의 기술적 이해도를 높이는 데 기여했다. `Header.jsx` 충돌을 직접 해결하면서 merge 충돌의 원인이 작업 범위 조율 부족에 있다는 것을 파악했고, 이후 브랜치 전략과 파일 담당 범위를 명확히 정리해 팀의 협업 흐름을 개선했다. 문서 업데이트, assets 정리, 배포 URL 수정 같은 기반 작업들을 직접 챙기며 팀원들이 핵심 기능에 집중할 수 있는 환경을 만들었고, 리더는 눈에 보이는 기능만큼 이런 기반을 다지는 역할도 중요하다는 것을 이번 프로젝트를 통해 배웠다.

### 손은애

> 프로필 및 사용자 관리 기능을 구현하면서 단순히 화면을 구성하는 것을 넘어, 데이터 변화에 따라 UI가 자연스럽게 반응하도록 설계하는 경험을 할 수 있었습니다. 팔로우/언팔로우, 프로필 수정, 게시글 및 상품 조건부 렌더링처럼 상태 변화가 많은 기능을 구현하면서 사용자 데이터와 UI 상태를 연결하는 구조에 대해 고민하게 되었습니다. 또한 프로필 수정 반영 문제나 채팅방 초대/나가기 과정에서 발생한 이슈를 해결하며 데이터 흐름과 상태 업데이트 과정의 중요성을 직접 체감할 수 있었습니다.
채팅 기능에서는 메시지 날짜 및 시간 표시 로직을 개선하고 메시지 검색 기능을 추가하는 등 사용자 경험을 고려한 기능 개선 작업도 진행했습니다. 게시글 업로드 후 상세 페이지로 이동하도록 흐름을 개선하고 폼 상태 관리와 유효성 검사 로직을 정리하면서 사용자 인터랙션이 자연스럽게 이어지도록 구조를 다듬는 과정을 경험했습니다.
