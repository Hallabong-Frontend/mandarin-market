...
### 3. AI 기반 초기 설계 시 SVG 하드 코딩으로 인한 리팩토링 어려움
...
**해결**  
수많은 컴포넌트 내에 하드 코딩된 `<svg>` 태그들을 일일이 찾아내어 별도의 외부 파일이나 React 컴포넌트로 분리하고, 이를 styled-components에서 불러와서 속성을 제어할 수 있도록 구조를 전면 개편했습니다.
