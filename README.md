...
프로젝트에서 여러 화면에 공통으로 사용되는 `Header` 컴포넌트를 구현하면서  
헤더 구조를 어떤 방식으로 설계하는 것이 적절한지 고민했습니다.

초기에는 `type` 값을 기준으로 내부에서 조건 분기를 통해  
화면별 헤더 UI를 재사용하는 방식을 사용했습니다.
...
하지만 개발이 진행되면서 헤더에 들어가는 요소가 점점 다양해졌습니다.  
...
이 방식은 Header 내부 요소를 각각 독립적인 컴포넌트로 분리하여  
필요한 요소를 조합하는 방식으로 사용할 수 있습니다.

문자열 기반 분기 대신 컴포넌트 조합 방식으로 UI를 구성할 수 있기 때문에  
다음과 같은 장점이 있다고 판단했습니다.

- 문자열 기반 조건 분기를 줄일 수 있음
- 화면별 요구사항에 따라 Header 구성을 유연하게 조합 가능
- 컴포넌트 단위 재사용성과 확장성이 높아짐

하지만 프로젝트가 이미 **개발 후반 단계**였고  
기존 Header 구조가 안정적으로 동작하고 있었기 때문에  
...
대신 향후 프로젝트 규모가 커지거나  
Header 조합 방식이 더 복잡해질 경우에는  
`Compound Component` 패턴을 적용하는 것이 더 적절한 구조라고 판단했습니다.

이번 경험을 통해 단순히 새로운 패턴을 적용하는 것보다  
**프로젝트의 단계와 리소스를 고려하여 기술 선택을 하는 것이 중요하다는 점을 배울 수 있었습니다.**
